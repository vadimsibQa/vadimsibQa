...
public class Box<T extends Fruit> {
    ...
    void moveToOtherBox(Box<T> box) {
        box.getList().addAll(fruitList);
        fruitList.clear();
    }
    ...
    void checkBoxState() {
        if (fruitList.isEmpty()) {
            ...
            System.out.println("В коробке лежит: " + fruitList.size() + " " + fruitList.getFirst().toString());
        }
    }
    ...
    float getWeight() {
        if (fruitList.isEmpty()) {
            ...
            return fruitList.size() * fruitList.getFirst().getWeight();
        }
    }
